...
public class CustomDijkstraFactoryPathFinder extends AbstractFactoryPathFinder<GridGraph, RectangularVertex> implements Serializable {
	...
	public List<Position> findPath(final Component sourceComponent,
		...
		for (final Vertex vertex : shortestPath) {
			if (vertex != startVertex) {
				shortestPathPositions.add(( (RectangularVertex) vertex ).getPosition());
			}
		}
	...
	@Override
	protected RectangularVertex getVertex(final int xIndex, 
									 final int yIndex) {
		final Iterator<RectangularVertex> iterator = getGraphVertexesIterator();
		final int resolution = getResolution();
		final Position position = new Position(xIndex * resolution, yIndex * resolution);

		while (iterator.hasNext()) {
			final RectangularVertex squareVertex = iterator.next();
	...
	@Override
	protected float overlayedSurface(final RectangularVertex vertex, 
	...
	@Override
	protected Iterator<RectangularVertex> getGraphVertexesIterator() {
	...
	protected boolean addVertex(final int xCoordinate,
		...
		final int resolution = getResolution();
		final Vertex vertex = new RectangularVertex("(" + xCoordinate / resolution + ", "+ yCoordinate / resolution + ")",
											   xCoordinate,
											   yCoordinate,
											   resolution);
	...
	@Override
	protected boolean addEdge(final RectangularVertex vertex1,
							  final RectangularVertex vertex2) {
		final GridGraph graph = getGraph();
	...
	@Override
	protected int getxCoordinate(final RectangularVertex vertex) {
		return vertex.getxCoordinate();
	}

	@Override
	protected int getyCoordinate(final RectangularVertex vertex) {
